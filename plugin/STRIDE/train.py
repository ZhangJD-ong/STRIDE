...
if __name__ == '__main__':
    ...
    result_path = os.path.join(os.getcwd(),opt.checkpoints_dir,opt.task_name)
    optimizer = optim.Adam(model.parameters(), lr=opt.lr, betas=(0.5, 0.999))
    model_save_path = os.path.join(result_path,'model')
    mkdir(model_save_path)

    logger_save_path = os.path.join(result_path,'logger')
    mkdir(logger_save_path)
    log_train = Train_Logger(logger_save_path,"train_log")
    prev_time = time.time()
