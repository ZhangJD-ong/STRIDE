...
def train_preprocess_lessMemoryMulStacks(args):
    ...
    gap_x = args.gap_x
    # gap_t2 = args.gap_t*2
    im_folder = os.path.join(os.getcwd(),args.datapath)
...
def singlebatch_test_save(single_coordinate, output_image, raw_image):
    ...
    o_patch = output_image[patch_start_s:patch_end_s, patch_start_h:patch_end_h, patch_start_w:patch_end_w]
    raw_patch = raw_image[patch_start_s:patch_end_s, patch_start_h:patch_end_h, patch_start_w:patch_end_w]

    if len(o_patch.shape) == 2:
        o_patch = o_patch[np.newaxis, :, :]
        raw_patch = raw_patch[np.newaxis, :, :]

    return o_patch, raw_patch, stack_start_w, stack_end_w, stack_start_h, stack_end_h, stack_start_s, stack_end_s
...
def multibatch_test_save(single_coordinate,id,output_image,raw_image):
    ...
    o_patch = output_image_id[patch_start_s:patch_end_s, patch_start_h:patch_end_h, patch_start_w:patch_end_w]
...
def test_preprocess_lessMemoryNoTail_chooseOne (args, N, iterations):
    patch_y = args.patch_y
    patch_x = args.patch_x
    ...
    if iterations == 0:
        ...
        im_dir = os.path.join(args.iter_path)
        noise_im = tiff.imread(im_dir)
    ...
    if args.image_type == 'xyt' and args.inference_mode_2D == True:
        num_s_all = whole_t-patch_t2
    else:
        num_s_all = num_s

    for z in range(0, num_s_all):
        for x in range(0,num_h):
            for y in range(0,num_w):
                ...
                if num_w ==1:
                    ...
                    single_coordinate['stack_end_w'] = y * gap_x + patch_x
                ...
                if num_h ==1:
                    ...
                    single_coordinate['stack_end_h'] = x * gap_y + patch_y
                ...
                if args.image_type == 'xyt' and args.inference_mode_2D == True:
                    if z == 0:
                        ...
                        single_coordinate['stack_end_s'] = z + patch_t2 // 2
                        single_coordinate['patch_start_s'] = 0
                        single_coordinate['patch_end_s'] = patch_t2 // 2
                    elif z == whole_t - patch_t2 - 1:
                        single_coordinate['stack_start_s'] = whole_t - patch_t2 // 2 - 2
                        single_coordinate['stack_end_s'] = whole_t
                        single_coordinate['patch_start_s'] = patch_t2 - patch_t2 // 2 - 2
                        single_coordinate['patch_end_s'] = patch_t2
                    else:
                        single_coordinate['stack_start_s'] = z + patch_t2 // 2-1
                        single_coordinate['stack_end_s'] = z + patch_t2 // 2
                        single_coordinate['patch_start_s'] = patch_t2 // 2-1
                        single_coordinate['patch_end_s'] = patch_t2 // 2
                ...
                # noise_patch1 = noise_im[init_s:end_s,init_h:end_h,init_w:end_w]
                patch_name = im_name+'_x'+str(x)+'_y'+str(y)+'_z'+str(z)
                # train_raw.append(noise_patch1.transpose(1,2,0))
                name_list.append(patch_name)
                # print(' single_coordinate -----> ',single_coordinate)
                coordinate_list[patch_name] = single_coordinate
